...
namespace SpikersArchipelago.Players
{
    public class ArchipelagoPlayer : ModPlayer
    {
        ...
        public override void OnEnterWorld()
        {
            ...
            {
                if (achievedWhileLoading.Contains(achievement.Value.Name))
                    continue;
                ...
                foreach (var condition in conditions)
                {
                    ...
                    if (condition.Value is CustomIntCondition intCondition)
                        intCondition.Value = serCondition.Get<int>("int");
                    if (condition.Value is CustomFloatCondition floatCondition)
                        floatCondition.Value = serCondition.Get<float>("float");
            ...
            {
                var mod = ModContent.GetInstance<SpikersArchipelago>();
        ...
        public override void SaveData(TagCompound tag)
        {
            ...
            foreach (var achievement in achievements)
            {
                ...
                foreach (var condition in conditions)
                {
                    ...
                    if (condition.Value is CustomIntCondition intCondition)
                        serCondition["int"] = intCondition.Value;
                    if (condition.Value is CustomFloatCondition floatCondition)
                        serCondition["float"] = floatCondition.Value;
        ...
        {
            if (damageSource.SourceCustomReason != null && damageSource.SourceCustomReason.StartsWith("[DeathLink]"))
                return;
        ...
        {
            if (vendor.type == ModContent.NPCType<CollectionNPC>() && !receivedRewards.Contains(item.type))
                receivedRewards.Add(item.type);
        }
