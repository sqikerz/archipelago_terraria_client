...
using SpikersArchipelago.HardmodeItem;
using SpikersArchipelago.NPCs;
using SpikersArchipelago.Players;
...
namespace SpikersArchipelago.Systems
{
    class ArchipelagoSystem : ModSystem
    {
        ...
        public override void OnWorldLoad()
        {
            ...
            if (success.SlotData.TryGetValue("randomize_npcs", out var rawValue) && rawValue is JArray jArr)
            {
                string[] randomizedNPCnames = jArr.ToObject<string[]>();

                if (randomizedNPCnames.Length > 0)
                {
                    world.randomizedNPCs = randomizedNPCnames
                        .Select(name => npcNameToID[name])
                        .ToImmutableHashSet();
            ...
            {
                world.randomizedNPCs = ImmutableHashSet<int>.Empty;
            }

            session.slot = success.Slot;

            foreach (var location in world.locationBacklog)
        ...
        public void Collect(string item)
        {
            ...
            switch (item)
            {
                case "Reward: Torch God's Favor":
                    GiveItem(ItemID.TorchGodsFavor);
                    break;
                case "Post-King Slime":
                    BossFlag(ref NPC.downedSlimeKing, NPCID.KingSlime);
                    break;
                case "Post-Eye of Cthulhu":
                    BossFlag(ref NPC.downedBoss1, NPCID.EyeofCthulhu);
                    break;
                case "Post-Evil Boss":
                    BossFlag(ref NPC.downedBoss2, NPCID.EaterofWorldsHead);
                    break;
                case "Post-Old One's Army Tier 1":
                    DD2Event.DownedInvasionT1 = true;
                    break;
                case "Post-Goblin Army":
                    NPC.downedGoblins = true;
                    break;
                case "Post-Queen Bee":
                    BossFlag(ref NPC.downedQueenBee, NPCID.QueenBee);
                    break;
                case "Post-Skeletron":
                    BossFlag(ref NPC.downedBoss3, NPCID.SkeletronHead);
                    break;
                case "Post-Deerclops":
                    BossFlag(ref NPC.downedDeerclops, NPCID.Deerclops);
                    break;
                case "Hardmode":
                    ...
                    break;
                case "Post-Pirate Invasion":
                    NPC.downedPirates = true;
                    break;
                case "Post-Queen Slime":
                    BossFlag(ref NPC.downedQueenSlime, NPCID.QueenSlimeBoss);
                    break;
                case "Post-The Twins":
                    ...
                    if (NPC.AnyNPCs(NPCID.Retinazer))
                    {
                        if (NPC.AnyNPCs(NPCID.Spazmatism))
                        {
                            ...
                            if (ModLoader.HasMod("CalamityMod"))
                                ModContent.GetInstance<CalamitySystem>().SpawnMechOres();
                            NPC.downedMechBoss2 = NPC.downedMechBossAny = true;
                        }
                        else
                            BossFlag(set, NPCID.Retinazer);
                    }
                    else
                        BossFlag(set, NPCID.Spazmatism);
                    break;
                case "Post-Old One's Army Tier 2":
                    DD2Event.DownedInvasionT2 = true;
                    break;
                case "Post-The Destroyer":
                    BossFlag(() => NPC.downedMechBoss1 = NPC.downedMechBossAny = true, NPCID.TheDestroyer);
                    break;
                case "Post-Skeletron Prime":
                    BossFlag(() => NPC.downedMechBoss3 = NPC.downedMechBossAny = true, NPCID.SkeletronPrime);
                    break;
                case "Post-Plantera":
                    BossFlag(ref NPC.downedPlantBoss, NPCID.Plantera);
                    break;
                case "Post-Golem":
                    BossFlag(ref NPC.downedGolemBoss, NPCID.Golem);
                    break;
                case "Post-Old One's Army Tier 3":
                    DD2Event.DownedInvasionT3 = true;
                    break;
                case "Post-Martian Madness":
                    NPC.downedMartians = true;
                    break;
                case "Post-Duke Fishron":
                    BossFlag(ref NPC.downedFishron, NPCID.DukeFishron);
                    break;
                case "Post-Mourning Wood":
                    BossFlag(ref NPC.downedHalloweenTree, NPCID.MourningWood);
                    break;
                case "Post-Pumpking":
                    BossFlag(ref NPC.downedHalloweenKing, NPCID.Pumpking);
                    break;
                case "Post-Everscream":
                    BossFlag(ref NPC.downedChristmasTree, NPCID.Everscream);
                    break;
                case "Post-Santa-NK1":
                    BossFlag(ref NPC.downedChristmasSantank, NPCID.SantaNK1);
                    break;
                case "Post-Ice Queen":
                    BossFlag(ref NPC.downedChristmasIceQueen, NPCID.IceQueen);
                    break;
                case "Post-Frost Legion":
                    NPC.downedFrost = true;
                    break;
                case "Post-Empress of Light":
                    BossFlag(ref NPC.downedEmpressOfLight, NPCID.HallowBoss);
                    break;
                case "Post-Lunatic Cultist":
                    BossFlag(ref NPC.downedAncientCultist, NPCID.CultistBoss);
                    break;
                case "Post-Lunar Events":
                    NPC.downedTowerNebula = NPC.downedTowerSolar = NPC.downedTowerStardust = NPC.downedTowerVortex = true;
                    break;
                case "Post-Moon Lord":
                    BossFlag(ref NPC.downedMoonlord, NPCID.MoonLordCore);
                    break;
                case "Post-Desert Scourge":
                    ModContent.GetInstance<CalamitySystem>().CalamityOnKillDesertScourge();
                    break;
                case "Post-Giant Clam":
                    ModContent.GetInstance<CalamitySystem>().CalamityOnKillGiantClam(false);
                    break;
                case "Post-Acid Rain Tier 1":
                    ModContent.GetInstance<CalamitySystem>().CalamityAcidRainTier1Downed();
                    break;
                case "Post-Crabulon":
                    ModContent.GetInstance<CalamitySystem>().CalamityOnKillCrabulon();
                    break;
                case "Post-The Hive Mind":
                    ModContent.GetInstance<CalamitySystem>().CalamityOnKillTheHiveMind();
                    break;
                case "Post-The Perforators":
                    ModContent.GetInstance<CalamitySystem>().CalamityOnKillThePerforators();
                    break;
                case "Post-The Slime God":
                    ModContent.GetInstance<CalamitySystem>().CalamityOnKillTheSlimeGod();
                    break;
                case "Post-Dreadnautilus":
                    ModContent.GetInstance<CalamitySystem>().CalamityDreadnautilusDowned();
                    break;
                case "Post-Hardmode Giant Clam":
                    ModContent.GetInstance<CalamitySystem>().CalamityOnKillGiantClam(true);
                    break;
                case "Post-Aquatic Scourge":
                    ModContent.GetInstance<CalamitySystem>().CalamityOnKillAquaticScourge();
                    break;
                case "Post-Cragmaw Mire":
                    ModContent.GetInstance<CalamitySystem>().CalamityOnKillCragmawMire();
                    break;
                case "Post-Acid Rain Tier 2":
                    ModContent.GetInstance<CalamitySystem>().CalamityAcidRainTier2Downed();
                    break;
                case "Post-Brimstone Elemental":
                    ModContent.GetInstance<CalamitySystem>().CalamityOnKillBrimstoneElemental();
                    break;
                case "Post-Cryogen":
                    ModContent.GetInstance<CalamitySystem>().CalamityOnKillCryogen();
                    break;
                case "Post-Calamitas Clone":
                    ModContent.GetInstance<CalamitySystem>().CalamityOnKillCalamitasClone();
                    break;
                case "Post-Great Sand Shark":
                    ModContent.GetInstance<CalamitySystem>().CalamityOnKillGreatSandShark();
                    break;
                case "Post-Leviathan and Anahita":
                    ModContent.GetInstance<CalamitySystem>().CalamityOnKillLeviathanAndAnahita();
                    break;
                case "Post-Astrum Aureus":
                    ModContent.GetInstance<CalamitySystem>().CalamityOnKillAstrumAureus();
                    break;
                case "Post-The Plaguebringer Goliath":
                    ModContent.GetInstance<CalamitySystem>().CalamityOnKillThePlaguebringerGoliath();
                    break;
                case "Post-Ravager":
                    ModContent.GetInstance<CalamitySystem>().CalamityOnKillRavager();
                    break;
                case "Post-Astrum Deus":
                    ModContent.GetInstance<CalamitySystem>().CalamityOnKillAstrumDeus();
                    break;
                case "Post-Profaned Guardians":
                    ModContent.GetInstance<CalamitySystem>().CalamityOnKillProfanedGuardians();
                    break;
                case "Post-The Dragonfolly":
                    ModContent.GetInstance<CalamitySystem>().CalamityOnKillTheDragonfolly();
                    break;
                case "Post-Providence, the Profaned Goddess":
                    ModContent.GetInstance<CalamitySystem>().CalamityOnKillProvidenceTheProfanedGoddess();
                    break;
                case "Post-Storm Weaver":
                    ModContent.GetInstance<CalamitySystem>().CalamityOnKillStormWeaver();
                    break;
                case "Post-Ceaseless Void":
                    ModContent.GetInstance<CalamitySystem>().CalamityOnKillCeaselessVoid();
                    break;
                case "Post-Signus, Envoy of the Devourer":
                    ModContent.GetInstance<CalamitySystem>().CalamityOnKillSignusEnvoyOfTheDevourer();
                    break;
                case "Post-Polterghast":
                    ModContent.GetInstance<CalamitySystem>().CalamityOnKillPolterghast();
                    break;
                case "Post-Mauler":
                    ModContent.GetInstance<CalamitySystem>().CalamityOnKillMauler();
                    break;
                case "Post-Nuclear Terror":
                    ModContent.GetInstance<CalamitySystem>().CalamityOnKillNuclearTerror();
                    break;
                case "Post-The Old Duke":
                    ModContent.GetInstance<CalamitySystem>().CalamityOnKillTheOldDuke();
                    break;
                case "Post-The Devourer of Gods":
                    ModContent.GetInstance<CalamitySystem>().CalamityOnKillTheDevourerOfGods();
                    break;
                case "Post-Yharon, Dragon of Rebirth":
                    ModContent.GetInstance<CalamitySystem>().CalamityOnKillYharonDragonOfRebirth();
                    break;
                case "Post-Exo Mechs":
                    ModContent.GetInstance<CalamitySystem>().CalamityOnKillExoMechs();
                    break;
                case "Post-Supreme Witch, Calamitas":
                    ModContent.GetInstance<CalamitySystem>().CalamityOnKillSupremeWitchCalamitas();
                    break;
                case "Post-Primordial Wyrm":
                    ModContent.GetInstance<CalamitySystem>().CalamityPrimordialWyrmDowned();
                    break;
                case "Post-Boss Rush":
                    ModContent.GetInstance<CalamitySystem>().CalamityBossRushDowned();
                    break;
                case "Reward: Hermes Boots":
                    GiveItem(ItemID.HermesBoots);
                    break;
                case "Reward: Magic Mirror":
                    GiveItem(ItemID.MagicMirror);
                    break;
                case "Reward: Demon Conch":
                    GiveItem(ItemID.DemonConch);
                    break;
                case "Reward: Magic Conch":
                    GiveItem(ItemID.MagicConch);
                    break;
                case "Reward: Grappling Hook":
                    GiveItem(ItemID.GrapplingHook);
                    break;
                case "Reward: Cloud in a Bottle":
                    GiveItem(ItemID.CloudinaBottle);
                    break;
                case "Reward: Climbing Claws":
                    GiveItem(ItemID.ClimbingClaws);
                    break;
                case "Reward: Ancient Chisel":
                    GiveItem(ItemID.AncientChisel);
                    break;
                case "Reward: Fledgling Wings":
                    GiveItem(ItemID.CreativeWings);
                    break;
                case "Reward: Rod of Discord":
                    GiveItem(ItemID.RodofDiscord);
                    break;
                case "Reward: Aglet":
                    GiveItem(ItemID.Aglet);
                    break;
                case "Reward: Anklet of the Wind":
                    GiveItem(ItemID.AnkletoftheWind);
                    break;
                case "Reward: Ice Skates":
                    GiveItem(ItemID.IceSkates);
                    break;
                case "Reward: Lava Charm":
                    GiveItem(ItemID.LavaCharm);
                    break;
                case "Reward: Water Walking Boots":
                    GiveItem(ItemID.WaterWalkingBoots);
                    break;
                case "Reward: Flipper":
                    GiveItem(ItemID.Flipper);
                    break;
                case "Reward: Frog Leg":
                    GiveItem(ItemID.FrogLeg);
                    break;
                case "Reward: Shoe Spikes":
                    GiveItem(ItemID.ShoeSpikes);
                    break;
                case "Reward: Tabi":
                    GiveItem(ItemID.Tabi);
                    break;
                case "Reward: Black Belt":
                    GiveItem(ItemID.BlackBelt);
                    break;
                case "Reward: Flying Carpet":
                    GiveItem(ItemID.FlyingCarpet);
                    break;
                case "Reward: Moon Charm":
                    GiveItem(ItemID.MoonCharm);
                    break;
                case "Reward: Neptune's Shell":
                    GiveItem(ItemID.NeptunesShell);
                    break;
                case "Reward: Compass":
                    GiveItem(ItemID.Compass);
                    break;
                case "Reward: Depth Meter":
                    GiveItem(ItemID.DepthMeter);
                    break;
                case "Reward: Radar":
                    GiveItem(ItemID.Radar);
                    break;
                case "Reward: Tally Counter":
                    GiveItem(ItemID.TallyCounter);
                    break;
                case "Reward: Lifeform Analyzer":
                    GiveItem(ItemID.LifeformAnalyzer);
                    break;
                case "Reward: DPS Meter":
                    GiveItem(ItemID.DPSMeter);
                    break;
                case "Reward: Stopwatch":
                    GiveItem(ItemID.Stopwatch);
                    break;
                case "Reward: Metal Detector":
                    GiveItem(ItemID.MetalDetector);
                    break;
                case "Reward: Fisherman's Pocket Guide":
                    GiveItem(ItemID.FishermansGuide);
                    break;
                case "Reward: Weather Radio":
                    GiveItem(ItemID.WeatherRadio);
                    break;
                case "Reward: Sextant":
                    GiveItem(ItemID.Sextant);
                    break;
                case "Reward: Band of Regeneration":
                    GiveItem(ItemID.BandofRegeneration);
                    break;
                case "Reward: Celestial Magnet":
                    GiveItem(ItemID.CelestialMagnet);
                    break;
                case "Reward: Nature's Gift":
                    GiveItem(ItemID.NaturesGift);
                    break;
                case "Reward: Philosopher's Stone":
                    GiveItem(ItemID.PhilosophersStone);
                    break;
                case "Reward: Cobalt Shield":
                    GiveItem(ItemID.CobaltShield);
                    break;
                case "Reward: Armor Polish":
                    GiveItem(ItemID.ArmorPolish);
                    break;
                case "Reward: Vitamins":
                    GiveItem(ItemID.Vitamins);
                    break;
                case "Reward: Bezoar":
                    GiveItem(ItemID.Bezoar);
                    break;
                case "Reward: Adhesive Bandage":
                    GiveItem(ItemID.AdhesiveBandage);
                    break;
                case "Reward: Megaphone":
                    GiveItem(ItemID.Megaphone);
                    break;
                case "Reward: Nazar":
                    GiveItem(ItemID.Nazar);
                    break;
                case "Reward: Fast Clock":
                    GiveItem(ItemID.FastClock);
                    break;
                case "Reward: Trifold Map":
                    GiveItem(ItemID.TrifoldMap);
                    break;
                case "Reward: Blindfold":
                    GiveItem(ItemID.Blindfold);
                    break;
                case "Reward: Pocket Mirror":
                    GiveItem(ItemID.PocketMirror);
                    break;
                case "Reward: Paladin's Shield":
                    GiveItem(ItemID.PaladinsShield);
                    break;
                case "Reward: Frozen Turtle Shell":
                    GiveItem(ItemID.FrozenTurtleShell);
                    break;
                case "Reward: Flesh Knuckles":
                    GiveItem(ItemID.FleshKnuckles);
                    break;
                case "Reward: Putrid Scent":
                    GiveItem(ItemID.PutridScent);
                    break;
                case "Reward: Feral Claws":
                    GiveItem(ItemID.FeralClaws);
                    break;
                case "Reward: Cross Necklace":
                    GiveItem(ItemID.CrossNecklace);
                    break;
                case "Reward: Star Cloak":
                    GiveItem(ItemID.StarCloak);
                    break;
                case "Reward: Titan Glove":
                    GiveItem(ItemID.TitanGlove);
                    break;
                case "Reward: Obsidian Rose":
                    GiveItem(ItemID.ObsidianRose);
                    break;
                case "Reward: Magma Stone":
                    GiveItem(ItemID.MagmaStone);
                    break;
                case "Reward: Shark Tooth Necklace":
                    GiveItem(ItemID.SharkToothNecklace);
                    break;
                case "Reward: Magic Quiver":
                    GiveItem(ItemID.MagicQuiver);
                    break;
                case "Reward: Rifle Scope":
                    GiveItem(ItemID.RifleScope);
                    break;
                case "Reward: Brick Layer":
                    GiveItem(ItemID.BrickLayer);
                    break;
                case "Reward: Extendo Grip":
                    GiveItem(ItemID.ExtendoGrip);
                    break;
                case "Reward: Paint Sprayer":
                    GiveItem(ItemID.PaintSprayer);
                    break;
                case "Reward: Portable Cement Mixer":
                    GiveItem(ItemID.PortableCementMixer);
                    break;
                case "Reward: Treasure Magnet":
                    GiveItem(ItemID.TreasureMagnet);
                    break;
                case "Reward: Step Stool":
                    GiveItem(ItemID.PortableStool);
                    break;
                case "Reward: Discount Card":
                    GiveItem(ItemID.DiscountCard);
                    break;
                case "Reward: Gold Ring":
                    GiveItem(ItemID.GoldRing);
                    break;
                case "Reward: Lucky Coin":
                    GiveItem(ItemID.LuckyCoin);
                    break;
                case "Reward: High Test Fishing Line":
                    GiveItem(ItemID.HighTestFishingLine);
                    break;
                case "Reward: Angler Earring":
                    GiveItem(ItemID.AnglerEarring);
                    break;
                case "Reward: Tackle Box":
                    GiveItem(ItemID.TackleBox);
                    break;
                case "Reward: Lavaproof Fishing Hook":
                    GiveItem(ItemID.LavaFishingHook);
                    break;
                case "Reward: Red Counterweight":
                    GiveItem(ItemID.RedCounterweight);
                    break;
                case "Reward: Yoyo Glove":
                    GiveItem(ItemID.YoYoGlove);
                    break;
                case "Reward: Coins":
                    GiveCoins();
                    break;
                case "Reward: Cosmolight":
                    ModContent.GetInstance<CalamitySystem>().GiveCosmolight();
                    break;
                case "Reward: Diving Helmet":
                    GiveItem(ItemID.DivingHelmet);
                    break;
                case "Reward: Jellyfish Necklace":
                    GiveItem(ItemID.JellyfishNecklace);
                    break;
                case "Reward: Corrupt Flask":
                    ModContent.GetInstance<CalamitySystem>().GiveCorruptFlask();
                    break;
                case "Reward: Crimson Flask":
                    ModContent.GetInstance<CalamitySystem>().GiveCrimsonFlask();
                    break;
                case "Reward: Craw Carapace":
                    ModContent.GetInstance<CalamitySystem>().GiveCrawCarapace();
                    break;
                case "Reward: Giant Shell":
                    ModContent.GetInstance<CalamitySystem>().GiveGiantShell();
                    break;
                case "Reward: Life Jelly":
                    ModContent.GetInstance<CalamitySystem>().GiveLifeJelly();
                    break;
                case "Reward: Vital Jelly":
                    ModContent.GetInstance<CalamitySystem>().GiveVitalJelly();
                    break;
                case "Reward: Cleansing Jelly":
                    ModContent.GetInstance<CalamitySystem>().GiveCleansingJelly();
                    break;
                case "Reward: Giant Tortoise Shell":
                    ModContent.GetInstance<CalamitySystem>().GiveGiantTortoiseShell();
                    break;
                case "Reward: Coin of Deceit":
                    ModContent.GetInstance<CalamitySystem>().GiveCoinOfDeceit();
                    break;
                case "Reward: Ink Bomb":
                    ModContent.GetInstance<CalamitySystem>().GiveInkBomb();
                    break;
                case "Reward: Voltaic Jelly":
                    ModContent.GetInstance<CalamitySystem>().GiveVoltaicJelly();
                    break;
                case "Reward: Wulfrum Battery":
                    ModContent.GetInstance<CalamitySystem>().GiveWulfrumBattery();
                    break;
                case "Reward: Luxor's Gift":
                    ModContent.GetInstance<CalamitySystem>().GiveLuxorsGift();
                    break;
                case "Reward: Raider's Talisman":
                    ModContent.GetInstance<CalamitySystem>().GiveRaidersTalisman();
                    break;
                case "Reward: Rotten Dogtooth":
                    ModContent.GetInstance<CalamitySystem>().GiveRottenDogtooth();
                    break;
                case "Reward: Scuttler's Jewel":
                    ModContent.GetInstance<CalamitySystem>().GiveScuttlersJewel();
                    break;
                case "Reward: Unstable Granite Core":
                    ModContent.GetInstance<CalamitySystem>().GiveUnstableGraniteCore();
                    break;
                case "Reward: Amidias' Spark":
                    ModContent.GetInstance<CalamitySystem>().GiveAmidiasSpark();
                    break;
                case "Reward: Ursa Sergeant":
                    ModContent.GetInstance<CalamitySystem>().GiveUrsaSergeant();
                    break;
                case "Reward: Trinket of Chi":
                    ModContent.GetInstance<CalamitySystem>().GiveTrinketOfChi();
                    break;
                case "Reward: The Transformer":
                    ModContent.GetInstance<CalamitySystem>().GiveTheTransformer();
                    break;
                case "Reward: Rover Drive":
                    ModContent.GetInstance<CalamitySystem>().GiveRoverDrive();
                    break;
                case "Reward: Marnite Repulsion Shield":
                    ModContent.GetInstance<CalamitySystem>().GiveMarniteRepulsionShield();
                    break;
                case "Reward: Frost Barrier":
                    ModContent.GetInstance<CalamitySystem>().GiveFrostBarrier();
                    break;
                case "Reward: Ancient Fossil":
                    ModContent.GetInstance<CalamitySystem>().GiveAncientFossil();
                    break;
                case "Reward: Spelunker's Amulet":
                    ModContent.GetInstance<CalamitySystem>().GiveSpelunkersAmulet();
                    break;
                case "Reward: Fungal Symbiote":
                    ModContent.GetInstance<CalamitySystem>().GiveFungalSymbiote();
                    break;
                case "Reward: Gladiator's Locket":
                    ModContent.GetInstance<CalamitySystem>().GiveGladiatorsLocket();
                    break;
                case "Reward: Wulfrum Acrobatics Pack":
                    ModContent.GetInstance<CalamitySystem>().GiveWulfrumAcrobaticsPack();
                    break;
                case "Reward: Depths Charm":
                    ModContent.GetInstance<CalamitySystem>().GiveDepthsCharm();
                    break;
                case "Reward: Anechoic Plating":
                    ModContent.GetInstance<CalamitySystem>().GiveAnechoicPlating();
                    break;
                case "Reward: Iron Boots":
                    ModContent.GetInstance<CalamitySystem>().GiveIronBoots();
                    break;
                case "Reward: Sprit Glyph":
                    ModContent.GetInstance<CalamitySystem>().GiveSpritGlyph();
                    break;
                case "Reward: Abyssal Amulet":
                    ModContent.GetInstance<CalamitySystem>().GiveAbyssalAmulet();
                    break;
                case "Reward: Life Crystal":
                    GiveItem(ItemID.LifeCrystal);
                    break;
                case "Reward: Enchanted Sword":
                    GiveItem(ItemID.EnchantedSword);
                    break;
                case "Reward: Starfury":
                    GiveItem(ItemID.Starfury);
                    break;
                case "Reward: Defender Medal":
                    GiveItem(ItemID.DefenderMedal);
                    break;
                case null:
                    break;
                default:
                    Chat($"Received unknown item: {item}");
                    break;
            }
        }
        public override void PostUpdateWorld()
        {
            ...
            for (var i = 0; i < session.locationQueue.Count; i++)
            {
                ...
                })
                {
                    ...
                    {
                        var item = session.locationQueue[i].Result.Values.FirstOrDefault();
                        if (item != null)
                            Chat($"Sent {item.ItemName} to {session.session.Players.GetPlayerAlias(item.Player)}!", Color.Red);
                    }
                    else
            ...
            foreach (var i in unqueue)
                session.locationQueue.RemoveAt(i);
            ...
            if (ModLoader.HasMod("CalamityMod"))
                ModContent.GetInstance<CalamitySystem>().CalamityPostUpdateWorld();

            if (session.victory)
                return;

            foreach (var goal in session.goals)
                if (!session.collectedLocations.Contains(goal))
                    return;
        ...
        public string[] DebugInfo()
        {
            ...
            else
            {
                ...
                if (session.locationQueue.Count > 0)
                {
                    ...
                    foreach (var location in session.locationQueue)
                        statuses.Add(location.Status switch
                        {
        ...
        public void Chat(string message, Color? color = null, int player = -1)
        {
            ...
            if (player == -1)
            {
                ...
                ChatHelper.SendChatMessageToClient(NetworkText.FromLiteral(message), c, player);
        }

        public void Chat(string[] messages, int player = -1, Color? color = null)
        {
            foreach (var message in messages)
                Chat(message, color, player);
        }
        ...
        public void QueueLocation(string locationName)
        {
            ...
            if (location == -1 || !session.session.Locations.AllMissingLocations.Contains(location))
                return;
        ...
        public void QueueLocationClient(string locationName)
        {
            ...
            var packet = ModContent.GetInstance<SpikersArchipelago>().GetPacket();
        ...
        public void ReceiveDeathlink(DeathLink death)
        {
            ...
            for (var i = 0; i < Main.maxPlayers; i++)
            {
                ...
                if (player.active && !player.dead)
                    player.Hurt(PlayerDeathReason.ByCustomReason(NetworkText.FromLiteral(message)), 999999, 1);
            }
        ...
        {
            if (ModLoader.HasMod("CalamityMod"))
                ModContent.GetInstance<CalamitySystem>().VanillaBossKilled(boss);
        }

        void GiveItem(int? item, Action<Player> giveItem)
        {
            if (item != null)
                world.receivedRewards.Add(item.Value);
                ...
                if (player.active)
                {
                    ...
                    if (item != null)
                    {
                        if (Main.netMode == NetmodeID.Server)
                        {
                            ...
                            player.GetModPlayer<ArchipelagoPlayer>().ReceivedReward(item.Value);
                    }
                }
            }
        }
        ...
        void GiveCoins()
        {
            ...
            {
                if (platinum > 0)
                    player.QuickSpawnItem(player.GetSource_GiftOrReward(), ItemID.PlatinumCoin, platinum);
                if (gold > 0)
                    player.QuickSpawnItem(player.GetSource_GiftOrReward(), ItemID.GoldCoin, gold);
                if (silver > 0)
                    player.QuickSpawnItem(player.GetSource_GiftOrReward(), ItemID.SilverCoin, silver);
            });
        }
        ...
        public override void ModifyHardmodeTasks(List<GenPass> list)
        {
            ...
            {
                if (ModLoader.HasMod("CalamityMod"))
                    ModContent.GetInstance<CalamitySystem>().CalamityStartHardmode();
            }));
        }
    }
}
